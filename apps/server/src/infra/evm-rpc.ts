 ...
 * pipeline can be driven by a mock in tests without a live EVM node.
 */
...
/** ethers-backed EVM JSON-RPC adapter. */
export class EvmRpcAdapter implements EvmRpc {
  private readonly provider: JsonRpcProvider;
  private traceSupported: boolean | null = null;

  constructor(private readonly config: AppConfig) {
    const req = new FetchRequest(config.chain.rpcUrl);
    req.timeout = config.requestTimeoutMs;
    const network = new Network(config.chain.network, config.chain.chainId);
...
export function createEvmRpc(config: AppConfig): EvmRpc {
  return new EvmRpcAdapter(config);
}
