...
import { EvmRpcError, type EvmRpc } from "../infra/evm-rpc.js";
...
export async function analyzeTransaction(
  ...
  if (body.network !== config.chain.network) {
    throw new AnalyzeValidationError(
      `Server is configured for ${config.chain.network}, request asked for ${body.network}`,
    );
  }
  ...
  try {
    ...
    if (e instanceof EvmRpcError) throw e;
  ...
  const decision = evaluatePolicy({
    ...
    usdcDecimals: config.chain.usdcDecimals,
    userWallet,
