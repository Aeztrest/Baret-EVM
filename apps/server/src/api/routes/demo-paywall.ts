...
const ANSWERS: Record<string, string> = {
  ...
  "explain usdc on-chain":
    "USDC is a standard ERC-20 stablecoin (6 decimals) usable for payments like x402 micropayments — fast and cheap on any modern EVM chain.",
};

function scrybeAnswer(q: string): string {
  const key = q.trim().toLowerCase();
  if (ANSWERS[key]) return ANSWERS[key]!;
  return `Scrybe (demo oracle): "${q}" — paid answers are served once the x402 USDC payment settles on-chain. This is a demo endpoint; wire a real model behind it for production.`;
}
