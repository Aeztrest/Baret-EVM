...
export function Home() {
  ...
  const refreshBalance = useCallback(async () => {
    ...
    try {
      ...
      // Only overwrite on a successful read — keep the last good value when a
      // balance comes back null (transient RPC hiccup), so it never blanks to 0.
      if (r.wei != null) setBalance(Number(formatEther(r.wei)));
      if (r.usdc != null) setUsdc(Number(r.usdc));
    } catch {
      /* keep last value */
    }
  }, [state?.address, rpc]);
  ...
  useEffect(() => {
    let cancelled = false;
    void refreshBalance();
    // Keep balances fresh while the popup is open (and self-heal a slow first load).
