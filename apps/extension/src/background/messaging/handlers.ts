...
const balanceHandler: Handler<"wallet.balance"> = async ({ address }) => {
  ...
  // Fetch native + USDC in parallel and independently — a failing token read
  // must not blank the native balance (and vice versa). Each retries; on total
  // failure we return null so the UI keeps its last value instead of showing 0.
  const [weiRes, usdcRes] = await Promise.allSettled([
    withRetry(() => provider.getBalance(target)),
    withRetry(async () => {
      const token = new Contract(chain.usdcAddress, ERC20_ABI, provider);
      return (await token.getFunction("balanceOf")(target)) as bigint;
    }),
  ]);

  const wei = weiRes.status === "fulfilled" ? weiRes.value.toString() : null;
  const usdc =
    usdcRes.status === "fulfilled"
      ? formatUnits(usdcRes.value, chain.usdcDecimals)
      : null;
